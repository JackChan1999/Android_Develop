...
- Handler 消息处理器，发送消息和处理消息
- Looper 轮询器，从messagequeue取消息，分发给handler处理
- Message 消息，数据的载体
...
3、what：这个字段可以说是消息的标志，在消息处理中，我们可以根据这个字段的不同的值进行不同的处理，类似于我们在处理Button事件时，通过switch(v.getId())判断是点击了哪个按钮。
...
## **Handler的post()、postAtTime()、postDelayed()**
...
创建Handler对象，必须有Looper对象，而Looper对象的初始化需要调用Looper.prepare()和Looper.loop()方法
